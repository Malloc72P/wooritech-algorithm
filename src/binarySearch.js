 ...
 * 해당 함수는 숫자에 대한 이진검색을 수행합니다
 * 비교함수인 comparator가 있으면 객체 이진검색을, 없으면 숫자 이진검색을 수행합니다.
 * @param array 이진 검색을 수행할 배열
 * @param target 위치를 찾고자 하는 대상 값
 * @param comparator 객체 이진 검색에 쓰이는 비교함수
 */
export default function binarySearch(array, target, comparator) {
    //comparator가 있으면 객체 이진검색, 없으면 숫자 이진검색을 수행
    if (comparator) {
        return objectBinarySearch(array, target, comparator);
    } else {
        return numberBinarySearch(array, target);
    }
}

function numberBinarySearch(array, target) {
...
function objectBinarySearch(array, targetObject, comparator) {
    //low는 최소값의 인덱스, high는 최대값의 인덱스.
    let low = 0;
    let high = array.length - 1;
    //최소값의 인덱스가 최대값의 인덱스보다 작거나 같을때만 수행
    while (low <= high) {
        //배열의 중간에 해당하는 인덱스를 구하되, 몫만 구할 수 있도록 내림 함수 사용
        const medium = Math.floor((low + high) / 2);
        const mediumObject = array[medium];
        if (comparator(mediumObject, targetObject) === 0) {//중간값이 대상과 같다면, 그 위치를 반환.
            return medium;
        } else if (comparator(mediumObject, targetObject) === 1) {//중간값이 대상보다 크다면, 최대값의 인덱스를 중간값의 이전 인덱스로 재설정.
            high = medium - 1;
        } else {//반대로 중간값이 작으면, 최소갑사의 인덱스를 중간값의 다음 값으로 재설정.
            low = medium + 1;
        }
